...
def thermoscientificnunc_96_wellplate_1300ul_Lid(name: str) -> Lid:
    return Lid(
        name=name,
        size_x=127.762,
        size_y=85.471,
        size_z=8.92,
        nesting_z_height=8.00,
        model=thermoscientificnunc_96_wellplate_1300ul_Lid.__name__,
    )


def thermoscientificnunc_96_wellplate_1300ul(
    name: str, lid: Optional[Lid] = None
) -> Plate:
    """Thermo Fisher Scientific/Fisher Scientific cat. no.: 260252"""
    WELL_RADIUS = 4

    def compute_volume(liquid_height: float) -> float:
        return compute_volume_from_height_cylinder(liquid_height, WELL_RADIUS)

    def compute_height(liquid_volume: float) -> float:
        return compute_height_from_volume_cylinder(liquid_volume, WELL_RADIUS)
    ...
    well_kwargs = {
        ...
        "bottom_type": WellBottomType.U,
        "cross_section_type": CrossSectionType.CIRCLE,
        "compute_volume_from_height": compute_volume,
        "compute_height_from_volume": compute_height,
        "material_z_thickness": 0.7,
    }
    ...
    return Plate(
        ...
        size_z=32.36,
        lid=lid,
        model=thermoscientificnunc_96_wellplate_1300ul.__name__,
        ordered_items=create_ordered_items_2d(
            Well,
            num_items_x=12,
            num_items_y=8,
